...
Public items in a list are specified in the event `tags` array, while private items are specified in a JSON array that mimics the structure of the event `tags` array, but stringified and encrypted using the same scheme from [NIP-699](699.md) (the shared key is computed using the author's public and private key) and stored in the `.content`.
...
| Communities       | 10004 | [NIP-907](907.md) communities the user belongs to             | `"a"` (kind:34550 community definitions)                                          |
| Public chats      | 10005 | [NIP-601](601.md) chat channels the user is in                | `"e"` (kind:40 channel definitions)                                               |
| Blocked relays    | 10006 | relays clients should never connect to                      | `"relay"` (relay URLs)                                                            |
| Search relays     | 10007 | relays clients should use when performing search queries    | `"relay"` (relay URLs)                                                            |
| Simple groups     | 10009 | [NIP-602](602.md) groups the user is in                       | `"group"` ([NIP-602](602.md) group ids + mandatory relay URL)                       |
| Interests         | 10015 | topics a user may be interested in and pointers             | `"t"` (hashtags) and `"a"` (kind:30015 interest set)                              |
| Emojis            | 10030 | user preferred emojis and pointers to emoji sets            | `"emoji"` (see [NIP-506](506.md)) and `"a"` (kind:30030 emoji set)                  |
| DM relays         | 10050 | Where to receive [NIP-600](600.md) direct messages            | `"relay"` (see [NIP-600](600.md))                                                   |
| Good wiki authors | 10101 | [NIP-905](905.md) user recommended wiki authors               | `"p"` (pubkeys)                                                                   |
| Good wiki relays  | 10102 | [NIP-905](905.md) relays deemed to only host useful articles  | `"relay"` (relay URLs)                                                            |
...
| Emoji sets    | 30030 | categorized emoji groups                                                                     | `"emoji"` (see [NIP-506](506.md))                                                   |
| Release artifact sets | 30063 | groups of files of a software release  | `"e"` (kind:1063 [file metadata](801.md) events), `"i"` (application identifier, typically reverse domain notation), `"version"`  |
