...
2. Client gets the remote user pubkey (either via a `bunker://` connection string or a NIP-201 login-flow; shown below)
...
The `content` field is a JSON-RPC-like message that is [NIP-699](699.md) encrypted and has the following structure:
...
Clients might choose to present a more familiar login flow, so users can type a NIP-201 address instead of a `bunker://` string.

When the user types a NIP-201 the client:

- Queries the `/.well-known/nostr.json` file from the domain for the NIP-201 address provided to get the user's pubkey (this is the **remote user pubkey**)
...
#### Remote signer discovery via NIP-910

In this last case, most often used to facilitate an OAuth-like signin flow, the client first looks for remote signers that have announced themselves via NIP-910 application handler events.

First the client will query for `kind: 31990` events that have a `k` tag of `24133`.

These are generally shown to a user, and once the user selects which remote signer to use and provides the remote user pubkey they want to use (via npub, pubkey, or NIP-201 value), the client can initiate a connection. Note that it's on the user to select the remote signer that is actually managing the remote key that they would like to use in this case. If the remote user pubkey is managed on another remote signer, the connection will fail.
