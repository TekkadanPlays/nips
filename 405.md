...
needed to route the event to its intended recipient, including the recipient's `p` tag or [NIP-104](104.md) proof of work.
...
Encryption is done following [NIP-402](402.md) on the JSON-encoded event. Place the encryption payload in the `.content`
...
to attach a certain amount of proof-of-work to the wrapper event per [NIP-104](104.md) in a bid to demonstrate that
