...
This NIP defines an encrypted direct messaging scheme using [NIP-402](402.md) encryption and [NIP-405](405.md) seals and gift wraps.
...
Tags that mention, quote and assemble threading structures MUST follow [NIP-101](101.md).
...
Following [NIP-405](405.md), the **unsigned** `kind:14` chat message must be sealed (`kind:13`) and then gift-wrapped (`kind:1059`) to each receiver and the sender individually.
...
The encryption algorithm MUST use the latest version of [NIP-402](402.md).
