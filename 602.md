...
This is the basic unit of a "microblog" reply note sent to a group. It's the same as `kind:11`, except for the fact that it must be used whenever it's in reply to some other note (either in reply to a `kind:11` or a `kind:12`). `kind:12` events SHOULD use NIP-101 markers, leaving an empty relay url:
...
`kind:10` SHOULD use NIP-101 markers, just like `kind:12`.
...
It's a NIP-207-like list of pubkeys that are members of the group. Relays might choose to not to publish this information or to restrict what pubkeys can fetch it.
...
A definition for kind `10009` was included in [NIP-207](207.md) that allows clients to store the list of groups a user wants to remember being in.
