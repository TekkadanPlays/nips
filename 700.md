...
- `a` is an optional event coordinate that allows tipping addressable events such as NIP-503 long-form notes.
...
A client can retrieve `zap receipt`s on events and pubkeys using a NIP-100 filter, for example `{"kinds": [9735], "#e": [...]}`. Zaps MUST be validated using the following steps:
