...
This specification defines video events representing a dedicated post of externally hosted content. These video events are _addressable_ and delete-requestable per [NIP-500](500.md).
...
The primary source of video information is the `imeta` tags which is defined in [NIP-511](511.md)
...
Additionally `service nip96` may be included to allow clients to search the authors NIP-802 server list to find the file using the hash.
