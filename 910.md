...
* `content` is an optional `metadata`-like stringified JSON object, as described in NIP-100. This content is useful when the pubkey creating the `kind:31990` is not an application. If `content` is empty, the `kind:0` of the pubkey should be used to display application information (e.g. name, picture, web, LUD16, etc.)
...
* `bech32` in a URL MUST be replaced by clients with the NIP-102-encoded entity that should be loaded by the application.

Multiple tags might be registered by the app, following NIP-102 nomenclature as the second value of the array.

A tag without a second value in the array SHOULD be considered a generic handler for any NIP-102 entity that is not handled by a different tag.
...
User B's client, not knowing how to handle a `kind:31337` might display the event using its `alt` tag (as described in NIP-105). When the user clicks on the event, the application queries for a handler for this `kind`:
